 ...
 * @brief D-Bus経由で受け取った信頼できない入力を検証する純粋関数群
 *
...
namespace qsnapper {
    namespace security {
        ...
        /**
         * @brief 一般的な絶対ファイルパスが安全かどうかを判定する
         *
         * 仕様:
         *   - 空文字列は拒否
         *   - 絶対パス (先頭 '/') でなければ拒否
         *   - 制御文字 (C0 / DEL / C1) を含む場合は拒否
         *   - 長さが PATH_MAX (4096) を超える場合は拒否
         *
         * @param path 検査対象の絶対パス
         * @return 安全な場合: true、拒否すべき場合: false
         */
         ...
         *       単一スラッシュ "/" を渡した場合は子要素が境界チェックで弾かれるため対象外と扱う
         *
